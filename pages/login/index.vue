                    ...
                    <div class="mt-12">
                        <form @submit.prevent="onSubmit">
                            <div>
                                ...
                                <input v-model="form.user_name" class="w-full text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" type="" placeholder="mike@gmail.com">
                            </div>
                            <div class="mt-8">
                                ...
                                <input  v-model="form.user_password" class="w-full text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" type="" placeholder="Enter your password">
                            </div>
                            <div class="mt-10">
                                <button class="bg-indigo-500 text-gray-100 p-4 w-full rounded-full tracking-wide
                                font-semibold font-display focus:outline-none focus:shadow-outline hover:bg-indigo-600
                                shadow-lg">
                                <p v-if="isloading">Loading.....</p> <p v-else>Login In</p>
                                </button>
                            </div>
                        </form>
...
<script setup>
const url = "http://192.168.34.181/amazon22/WS/user_login?";
...
const form = reactive({
  ...
  user_password: ""
    ...
    async function onSubmit(){
      ...
      isloading.value = ref(!isloading)
        const {data} = await useFetch(url,{
            method:'post',
            body: form 
        });

        if(data.value.settings.success == '1' ){
          isloading.value = false;
          auth.value.isAuthenticate = true          
        }else{
          alert(data.value.settings.message)
          isloading.value = false;
          auth.value.isAuthenticate = false
        }

        if(auth.value.isAuthenticate == true){
          navigateTo('/')
        }
