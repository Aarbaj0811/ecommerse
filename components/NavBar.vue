<template>
    <!-- <nav class="text-white px-4 sm:px-8 py-2 sm:py-3 bg-sky-300 h-15">
        <div class="my-2">
            
            <NuxtLink to="/" class="text-white px-4 sm:px-8 py-2 sm:py-3 bg-sky-700 hover:bg-yellow-800 ">Home</NuxtLink>
            <NuxtLink to="/about" class="text-white px-4 sm:px-8 py-2 sm:py-3 bg-sky-700 hover:bg-yellow-800 m-2">About</NuxtLink>
            <NuxtLink to="/profile" class="text-white px-4 sm:px-8 py-2 sm:py-3 bg-sky-700 hover:bg-yellow-800 m-2">Profile</NuxtLink>
            <NuxtLink to="/product" class="text-white px-4 sm:px-8 py-2 sm:py-3 bg-sky-700 hover:bg-yellow-800 m-2">Product</NuxtLink>
            <NuxtLink to="/product" class="text-white px-4 sm:px-8 py-2 sm:py-3 bg-sky-700 hover:bg-yellow-800 m-2">Cart ({{ cart.length }})</NuxtLink>
    ...
    <nav
      ...
      >
       ...
       <div class="hidden w-full md:flex md:items-center md:w-auto" id="menu">
          ...
          >
            <li>
              <NuxtLink to="/" class="md:p-4 py-2 block hover:text-purple-400">Home</NuxtLink>
            </li>
            <!-- <li>
              
              <NuxtLink to="/about" class="md:p-4 py-2 block hover:text-purple-400">About</NuxtLink>
            </li> 
            <li>
             
              <NuxtLink to="/profile" class="md:p-4 py-2 block hover:text-purple-400">Profile</NuxtLink>
            </li> -->
            <li>
              <NuxtLink to="/product" class="md:p-4 py-2 block hover:text-purple-400">Product</NuxtLink>
            </li>
            <li>
              <NuxtLink to="/product" class="md:p-4 py-2 block hover:text-purple-400 text-purple-500">Cart ({{ cart.length }})</NuxtLink>
            </li>
            <li class=" flex items-center " v-if=auth.isAuthenticate>
             
              <div class="flex items-center ">
              <div  class="relative ml-3 dropdown" x-data="{ open: false }">
                <div>
                  <button @click="open = !open" class="flex items-center max-w-xs text-sm text-white rounded-full focus:outline-none focus:shadow-solid" id="user-menu" aria-label="User menu" aria-haspopup="true" x-bind:aria-expanded="open" aria-expanded="true">
                    <img class=" dropdown w-10 h-10 rounded-full" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&amp;ixid=eyJhcHBfaWQiOjEyMDd9&amp;auto=format&amp;fit=facearea&amp;facepad=2&amp;w=256&amp;h=256&amp;q=80" alt="">
                  </button>
                </div>
                <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opaity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class=" dropdown text-gray-700 pt-1 absolute right-0 w-48 mt-2 origin-top-right rounded-md shadow-lg">
                  <div class="py-1 bg-white rounded-md shadow-xs dropdown-menu hidden ">
                    <a href="#" class="flex flex-row items-center px-4 py-2 text-sm text-gray-700 focus:text-gray-900 hover:text-gray-900 focus:outline-none hover:bg-gray-100 focus:bg-gray-100">
                      ...
                      <NuxtLink to="/profile" class=" ml-2">Your Profile</NuxtLink>
                    </a>
                    <a href="#" class="flex flex-row items-center px-4 py-2 text-sm text-gray-700 focus:text-gray-900 hover:text-gray-900 focus:outline-none hover:bg-gray-100 focus:bg-gray-100">
                    ...
                    <a href="#" class="flex flex-row items-center px-4 py-2 text-sm text-red-500 hover:text-red-700 hover:bg-red-100 focus:outline-none focus:text-red-700 focus:bg-red-100">
                      ...
                      <span class="ml-2" @click="">Sign out</span>
                    </a>
                  </div>
                </div>
              </div>
...
<script setup>
    const cart = useCart();
    const auth = useAuth();
    console.log( auth.value.isAuthenticate)

    function signOut(){
      auth.value.isAuthenticate = false
      navigateTo('/login')
    }
</script>
<style >
